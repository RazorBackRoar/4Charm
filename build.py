...
def main():
    ...
    try:
        ...
        if os.path.exists(os.path.join(APP_PATH, "Contents/MacOS/4Charm")):
            log(
                "⚠️  py2app reported failure but app bundle exists. Proceeding with caution.",
                YELLOW,
            )
        else:
            sys.exit(1)
    ...
    log(
        f"📝 Open {DMG_FINAL} manually and drag {APP_NAME}.app into /Applications when you're ready.",
        BLUE,
    )
